...
**This overturns the previously deliberate "surfaced, not prevented" stance** (see
`dev-docs/architecture/10-invariants/coordinates-and-slugs.md`) — but only for renames that flow
through `save()`. NetBox's bulk CSV import / bulk-edit use `QuerySet.bulk_update`, which fires **no**
`save()`/`post_save`, so a bulk rename bypasses these receivers entirely; that residue is exactly why
`_resolve_floor_location`/`health._check_floor_keys` stay as the backstop and why HEALTH-5 (a
...
class SlugRenameRemapper:
    ...
    One instance per rename, bound to the facility the renamed object resolves to
    (`facility_for_site`). The three public `dispatch_*` entry points pick the rename kind by the
    ...
    def _build_in_progress(self):
        ...
        from .imports import RenderRunner  # deferred: only touched on an actual rename
...
def _on_site_saved(sender, instance, created, **kwargs):
    ...
    facility = facility_for_site(instance)
    transaction.on_commit(lambda: _safely(
        lambda: SlugRenameRemapper(facility).dispatch_site(old, new)))
...
def _on_location_saved(sender, instance, created, **kwargs):
    ...
    transaction.on_commit(lambda: _safely(
        lambda: SlugRenameRemapper(facility).dispatch_location(site_slug, parent_slug, old, new)))
